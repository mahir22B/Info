...
#     try:
#         prompt = f"""Based on the following content, create an informative structure for an infographic about {parsed_content['title']}, the title should be strictly less than 3 words if longer.
...
# IMPORTANT: 
#         1. Ensure that you preserve the original context and meaning of the scraped text. Do not alter or misrepresent the information provided. Your task is to summarize and structure the content, not to change its meaning or intent.
#         2. Use a first-person perspective ("I") instead of referring to "the author" or "the writer".
...
#         response = requests.get(f'{SCRAPING_ROBOT_API_URL}?token={API_TOKEN}&url={url}')
#         response.raise_for_status()
...
#             return jsonify(infographic_data), 400
...
#     except Exception as e:
#         db.session.rollback()
#         app.logger.error(f"Error generating infographic: {str(e)}")
...
app = Flask(__name__)
CORS(app, resources={r"/api/*": {"origins": "http://www.instagraphix.pro"}}, supports_credentials=True)
app.secret_key = 'os.urandom(24).hex()'
oauth = OAuth(app)
...
def authorized():
    try:
        ...
        # Redirect to frontend
        return redirect('http://www.instagraphix.pro')
    except Exception as e:
        print(f"Error in Google callback: {str(e)}")
        return jsonify({'error': 'Authentication failed'}), 400
...
@app.route('/api/generate_infographic', methods=['POST'])
@cross_origin(origins=["http://www.instagraphix.pro"], supports_credentials=True)
@token_required
def generate_infographic(user):
    try:
        if user.credits < 1:
