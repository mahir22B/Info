...
const API_URL = "https://api.instagraphix.pro";
...
const GenerateFromScratch = ({ onCreditsUpdate }) => {
  ...
  const handleGenerate = async () => {
    setIsLoading(true);
    try {
      const response = await fetch('https://api.instagraphix.pro/api/generate_from_scratch', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      // Fetch config files for each infographic
      const infographicsWithConfig = await Promise.all(data.infographics.map(async (infographic) => {
        const configResponse = await fetch(`https://api.instagraphix.pro/api/get_config/${infographic.template_name}`);
        if (!configResponse.ok) {
          throw new Error(`Failed to fetch config for ${infographic.template_name}`);
        }
        const config = await configResponse.json();
...
const UrlInput = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await fetch('https://api.instagraphix.pro/api/generate_infographic', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleSaveCustomizations = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch('https://api.instagraphix.pro/api/finalize_infographic', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
